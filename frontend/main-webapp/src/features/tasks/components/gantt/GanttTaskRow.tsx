...
export default function GanttTaskRow({ task, isSelected, onClick }: GanttTaskRowProps) {
  ...
  return (
    <div
      onClick={onClick}
      className={`flex items-center h-12 border-b border-neutral-200 hover:bg-neutral-50 cursor-pointer transition-colors ${
        isSelected ? "bg-primary/5" : ""
      }`}
    >
      {/* Task Name */}
      <div className="flex-1 min-w-[180px] px-4 truncate">
        <span className="text-sm font-medium text-neutral-900">{task.name}</span>
      </div>

      {/* Owner */}
      <div className="w-[100px] px-3 flex justify-center">
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-medium overflow-hidden ${
            task.owner.avatar ? "" : getAvatarColor(task.owner.name)
          }`}
